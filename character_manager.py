...
def update_profile(character_name, profile_data):
        ...
        if field in merged_profile:
            if isinstance(merged_profile[field], str):
                merged_profile[field] = [
                    line.strip()
                    for line in merged_profile[field].split("\n")
                    if line.strip()
                ]
    ...
    if "dialogue_examples" in merged_profile and not isinstance(
        merged_profile["dialogue_examples"], list
    ):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Profile for {character_name} updated successfully",
        }
    except Exception as e:
...
def import_profile_from_json(json_data):
    ...
    try:
        ...
        if missing_fields:
            return {
                "error": f"Required fields missing in JSON: {', '.join(missing_fields)}"
            }
        ...
        for field in list_fields:
            if field in profile_data:
                ...
                if isinstance(profile_data[field], str):
                    profile_data[field] = [
                        line.strip()
                        for line in profile_data[field].split("\n")
                        if line.strip()
                    ]
                # Ensure it's a list
